...
export const PlanList = styled.div`
  display: flex;
  flex-wrap: wrap;
  align-items: stretch; /* tất cả item bằng chiều cao nhau */
  padding: 12px;
  border-radius: 12px;
...
export const WrapPlanCard = styled.div`
  flex: 1 1 25%;
  padding: 12px;
  display: flex; /* để PlanCard stretch full height */

  /* border-right chỉ hiện nếu không phải phần tử cuối của hàng */
...
export const PlanCard = styled.div<{ isDark?: boolean }>`
  flex: 1; /* chiếm hết chiều cao cha */
  display: flex;
  ...
  background-color: ${({ isDark }) =>
    isDark ? themeColors.secondaryDark : themeColors.newtralLightest};
  color: ${({ isDark }) =>
    isDark ? themeColors.newtralLightest : themeColors.newtralDark};
...
export const PlanPrice = styled.div<{ isDark?: boolean }>`
  ...
  color: ${({ isDark }) =>
    isDark ? themeColors.newtralLightest : themeColors.primary};
`;
