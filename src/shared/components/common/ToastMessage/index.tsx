...
const  ToastMessage:React.FC<ToastMessageProps> =({typeToast,message}) =>{
  const { t } = useTranslation("messages");
    ...
    const renderContentToast = useMemo(() =>{
        ...
        case ToastMessageType?.INFO:
            return  <S.ToastMessageWrap>
                <S.ToastIcon src={icSuccess} preview={false}/>
                <S.ToastContent>
                    <S.ToastLabel variant='body-text-larger' fontWeight={fontWeight?.semiBold}>{t('success')}</S.ToastLabel>
         ...
         case ToastMessageType?.ERROR:
            return  <S.ToastMessageWrap>
                ...
                <S.ToastContent>
                    <S.ToastLabel variant='body-text-larger' fontWeight={fontWeight?.semiBold}>{t('error')}</S.ToastLabel>
                    {message && <S.Toast>{message}</S.Toast>}
                </S.ToastContent>
                    </S.ToastMessageWrap>;
            default:
  ...
  return (
    <Fragment>{renderContentToast}</Fragment>
  )
}
